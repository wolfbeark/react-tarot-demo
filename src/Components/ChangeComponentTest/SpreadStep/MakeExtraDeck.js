...
import { HorizontalContainer } from "../../CustomStyles";
  ...
  ${OptionBtnBox} {
    ...
    & > button {
      ...
      background-color: ${(props) =>
        props.btncolor ? props.btncolor : "blue"};
    }
  }
`;
...
function MakeExtraDeck(props) {
  ...
  const [selectDeckControlArr, setSelectDeckControlArr] = useState(
    new Array(10)
  );
  ...
  const messageArr = ["입력 값이 올바르지 않습니다", "잠시만 기다려 주십시오"];
  let { imgTypeArr, setImgTypeArr } = props.imgTypeControler;
  let { hideBtnOnOffArr, setHideBtnOnOffArr } = props.hideBtnOnOffArrController;
  ...
  let { hideInfoArr, setHideInfoArr } = props.hideInfoArrController;
  ...
  const onMakeClickHandler = () => {
    if (isThereTotal === false) {
      ...
      if (
        (isClickedMajor === true && isClickedMinor === true) ||
        (isClickedMajor === true && isMinorAllTrue === true)
      ) {
  ...
  const onExtraNumberSubmit = (e) => {
    ...
    if (firstFlag && isThereTotal === false) {
      // 2차 범위 확인 및 번호 추출, 토탈아님
      // listSelectArr
      ...
      for (let i = 0; i < _listTempArr.length; i++) {
        ...
        for (
          let j = DeckNumberArr[type].startNum;
          j <= DeckNumberArr[type].endNum;
          j++
        ) {
    ...
    } else if (firstFlag && isThereTotal === true) {
      // 2차 범위 확인 및 번호 추출, 토탈임 (토탈 - 선택)
          ...
          <OptionContainer>
                  ...
                  <>
                    {deckClickCount === 0 ? (
                      ...
                      listSelectArr.map((a, i) => {
                        return <li key={"listSelect" + i}>{a.name}</li>;
                      })
                    )}
                  </>
                </SelectedList>
            ...
            <OptionBoxContainer>
              <OptionBtnBox
                ...
                animate={
                  isThereTotal === false ? "totalUnclicked" : "totalClicked"
                }
                whileHover="hover"
                whileTap="click"
                btncolor="#123456"
              ...
              >
                ...
                {canMakeExtraCard === true ? (
                  ...
                  >
                    ...
                    />
                    <ExtraSetNumberBtnBox setwidth={30} setheight={90}>
                      <OptionBtnBox
                        variants={optionBtnVariants}
