...
const DragCardContainer2 = styled(motion.div)`
  ...
  ${(props) => {
    if (props.modenumber === 0) {
      if (props.count < props.usersetnumber) {
        return css`
          left: ${(props) =>
            props.selectedposinfo.x - props.mainspreadposinfo.x}px;
          top: ${(props) =>
            props.selectedposinfo.y - props.mainspreadposinfo.y}px;
        `;
        ...
        if (props.isclickedlenormandtotal === "false") {
          ...
          if (
            props.count >= props.lenormandtotalidx[0] &&
            props.count <= props.lenormandtotalidx[35]
          ) {
            return css`
              left: calc(
                50% - ${(props) => props.selectedposinfo.width / 2}px +
                  ${(props) => props.modetempx}
              );
              top: calc(
                50% - ${(props) => props.selectedposinfo.height / 2}px +
                  ${(props) => props.modetempy}
              );
            `;
          } else {
            return css`
  ...
  ${(props) => {
    if (props.modeNumber === 2 && props.count === 8) {
      return css`
...
function DragCard(props) {
  ...
  const [selectedPosInfo, setSelectedPosInfo] = useState({
    x: 0,
    y: 0,
    width: 0,
    height: 0,
  ...
  let { isClickedLenormandTotal, setIsClickedLenormandTotal } =
  ...
  let _tempThreeX = ["+15%", "0%", "-15%"];

  let _tempThreeY = ["0%", "0%", "0%"];

  let tempHexaX = ["-30%", "-20%", "-40%", "-30%", "-40%", "-20%", "-30%"];
  let tempHexaY = [
    "-10%", // 7
    "-20%", // 6
    "-20%", // 5
  ...
  useEffect(() => {
    ...
    if (isClickedLenormandTotal === true) {
      ...
      if (_count >= _firstIdx && _count <= _lastIdx) {
    ...
    if (modeNumber !== 0) {
      ...
      switch (modeNumber) {
        ...
        case 3:
          ...
          //   }
          if (_count < userSetNumber) {
            setModeTempX(tempCelticX10[_count]);
            setModeTempY(tempCelticY10[_count]);
          }
          break;
        default:
          break;
  ...
  return (
    ...
    >
      <DragCardContainer2
        ...
        dragConstraints={
          isInSpreadZone === false ? props.refArr[0] : props.refArr[1]
        }
        dragMomentum={false}
        onDrag={(e) => {
          if (isThisCardHide === false) {
